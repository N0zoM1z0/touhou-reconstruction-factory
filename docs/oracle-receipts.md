...
| TH09 VC7.1 | one declared function extent | unconditional selected-unit compiler invocation | The strict COFF report must bind unit, address, claimed size, comparison extent, matched bytes, and reviewed relocations. It does not promote product closure or whole-image equality. |
| TH10 VC7.1 SP1 normal COFF | one explicitly normal-COFF function extent | unconditional selected-unit compiler invocation through game-bound headless Wine | The unit must declare `artifact_kind = "coff"`, a nonempty profile, and no `/GL`; the strict report binds unit, address, size, matched bytes, and reviewed relocations. It proves neither LTCG physical ownership nor whole-product closure. |
